...
# Set the working directory inside the container to `/xcrawl3r`.
...
# This includes all files from the current directory where the Dockerfile is located into the `/xcrawl3r` directory.
COPY . .
...
# Copy the compiled binary `xcrawl3r` from the `build-stage` stage.
# The binary is located at `/xcrawl3r/bin/xcrawl3r` in the build environment and is copied to `/usr/local/bin/` in the final image.
COPY --from=build-stage /xcrawl3r/bin/xcrawl3r /usr/local/bin/

# Set the default command to execute the `xcrawl3r` binary when the container starts.
# This means that when you run the container, it will automatically start the `xcrawl3r` tool.
ENTRYPOINT ["xcrawl3r"]